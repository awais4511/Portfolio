// Complete Enhanced Navbar Example with All Animations
// You can use this as a reference or replace your current Navbar.jsx with enhanced version
...
/**
 * FULLY ENHANCED NAVBAR WITH ALL ANIMATIONS
 * 
 * Features:
 * - Logo slide in + glow hover
 * - Navigation links stagger fade
 * - CTA button slide in + glow effect
...
export const EnhancedNavbar = () => {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      // 1. Logo Animation - Slide in + Scale
      gsap.fromTo(
        logoRef.current,
      ...
      // ============================================
      // ADVANCED ANIMATIONS (Optional - Uncomment to use)
      // ============================================
  ...
  return (
    ...
    >
      <nav className="container mx-auto px-4 sm:px-6 flex items-center justify-between">
        ...
        <div className="hidden md:flex items-center gap-1">
          ...
          >
            {navLinks.map((link, index) => (
              ...
              >
                {link.label}
                {/* Underline element (for underline animation) */}
        ...
        {/* Mobile Menu Button */}
        <button
          className="md:hidden p-2 text-foreground cursor-pointer transition-transform duration-300 hover:scale-110"
...
/*
// To enable Underline Animation:
1. Import: import { navLinkUnderline } from '@/components/advanced-navbar-animations';
2. Add in first useEffect inside gsap.context():
   navLinkUnderline('.nav-link');

// To enable Active Link Indicator:
1. Import: import { activeNavLink } from '@/components/advanced-navbar-animations';
2. Add in first useEffect inside gsap.context():
   activeNavLink('[id]', '.nav-link');

// To enable Button Glow:
1. Import: import { buttonGlowEffect } from '@/components/advanced-navbar-animations';
2. Add in first useEffect inside gsap.context():
   buttonGlowEffect(ctaButtonRef);

// To enable Hide Navbar on Scroll:
1. Import: import { navbarHideOnScroll } from '@/components/advanced-navbar-animations';
2. Add new useEffect:
   useEffect(() => {
...
// More blur on scroll:
gsap.to(headerRef.current, {
  backdropFilter: "blur(20px)", // Changed from 10px
  backgroundColor: "rgba(15, 23, 42, 0.9)", // Changed from 0.8
  duration: 0.3,
  overwrite: "auto",
});
*/
